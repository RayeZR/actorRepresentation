...
do
  ...
  if [ $tool == "ffprobe" ]; then
    ...
    echo "-show_frames -of csv=s=,:nk=0 -f lavfi 'movie=${inp_dir}/${file}, select=gt(scene\,${threshold})' > ${raw_output_dir}/${filename}.txt 2>&1"
    ffprobe -show_frames -of csv=s=,:nk=0 -f lavfi "movie=${inp_dir}/${file}, select=gt(scene\,${threshold})" > ${raw_output_dir}/${filename}".txt" 2>&1

    echo "python3 format_ffprobe.py --input_file ${raw_output_dir}/${filename}.txt --threshold $threshold --cmd $command"
    python3 format_ffprobe.py --input_file ${raw_output_dir}/${filename}.txt --threshold $threshold --cmd $command
  fi
  ...
  if [ $tool == "pyscenedetect" ]; then
    ...
    echo "python3 format_pyscenedetect.py --inp_file ${raw_output_dir}/${filename}'-Scenes.csv' --threshold $threshold --cmd $command"
