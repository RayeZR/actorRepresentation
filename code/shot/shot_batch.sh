...
do
  ...
  if [ $tool == "ffprobe" ]; then
    ...
    echo "python3 format_ffprobe.py --inp_file ${raw_output_dir}/${filename}.txt --threshold $threshold --cmd $command"
    python3 format_ffprobe.py --inp_file ${raw_output_dir}/${filename}.txt --threshold $threshold --cmd "$command"
  fi
