    ...
    for frame_id in range(num_frames):
        if res_openpose["frames"]["%s" % frame_id] == {}:
            ...
            if res_glib["frames"]["%s" % frame_id] != {}:
                ...
                for person, val in res_openpose["frames"]["%s" % frame_id].items():
                    res_openpose["frames"]["%s" % frame_id][person]["face_kpts"][68] = [-1, -1, -1]
                    res_openpose["frames"]["%s" % frame_id][person]["face_kpts"][69] = [-1, -1, -1]
