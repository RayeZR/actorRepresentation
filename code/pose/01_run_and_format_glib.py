"""
This script is to run glib and generate the pose-glib-raw.json.
Called by pose_batch.sh. Example command can be found in pose_batch.sh.
"""
...
def cal_on_screen_area(pose_kpts, face_kpts, ratio=1.2):
    x_list = [item[0] for item in [pose_kpts[i] for i in range(len(pose_kpts))]]
    x_list.extend(item[0] for item in [face_kpts[str(i)] for i in range(len(face_kpts))])
    y_list = [item[1] for item in [pose_kpts[i] for i in range(len(pose_kpts))]]
    y_list.extend(item[1] for item in [face_kpts[str(i)] for i in range(len(face_kpts))])
    x_list = [item for item in filter(lambda x: x != -1, x_list)]
    y_list = [item for item in filter(lambda x: x != -1, y_list)]
    ...
    while frame is not None:
        ...
        for (i, rect) in enumerate(rects):
            res_person = {"on_screen_area": -1,
                          "pose_kpts": dict(zip([idx for idx in range(25)], [[-1, -1, -1] for i in range(25)])),
                          "face_kpts": {}}  ## keep the same output format as openpose
            # res_person = {"on_screen_area": -1,
            #               "pose_kpts": {},
            #               "face_kpts": {}}
            ...
            res_person["face_kpts"][68] = [-1, -1, -1]
            res_person["face_kpts"][69] = [-1, -1, -1]

            res_person["on_screen_area"] = cal_on_screen_area(res_person["pose_kpts"], res_person["face_kpts"])
