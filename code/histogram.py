...
class Solver:
    ...
    def write(self, out_dir):
        ...
        idx = 0
        label = self.labels[0]
        writer = cv2.VideoWriter(os.path.join(out_dir, self.filename) + '_%03d.avi' % idx, self.fourcc, self.fps, (self.width, self.height))
        ret, frame = self.cap.read()
        i = 0
        info = '000.avi: '
        while frame is not None:
            if self.labels[i] == label:
                ...
                writer.write(frame)
                print(info)
                info = '%03d.avi: %d ' % (idx, self.labels[i])
            label = self.labels[i]
            i += 1
            # print(i)
            ret, frame = self.cap.read()
...
if __name__ == '__main__':
    ...
    inp_file = r'E:\I3S\actorRepresentation\data\PySceneDetect\04\04-Scene-001.mp4'
    out_dir = r'E:\I3S\actorRepresentation\data\PySceneDetect\hist_test_04-001'
    # num_bin = 16
    num_bin = 256
    num_center = 6
