...
class Custom_Dataset(torch.utils.data.Dataset):
    def __init__(self, inp_dir):
        ...
        self.imgs = sorted(os.listdir(inp_dir))
    ...
    def __getitem__(self, idx):
        img_dir = self.inp_dir + os.sep + self.imgs[idx]
...
#    data_loader, test_dataset = make_test_dataloader(cfg)
    inp_dir = r"data/test/03_10/"
    test_dataset = Custom_Dataset(inp_dir)
...
#    for i, (images, annos) in enumerate(data_loader):
    for (i, images) in enumerate(data_loader):
        ...
        if i % 1 == 0:
            img_save_dir = os.path.join(final_output_dir, '03_10')
            if not os.path.exists(img_save_dir):
                os.makedirs(img_save_dir)
            # prefix = '{}_{}'.format(os.path.join(final_output_dir, 'result_valid'), i)
            prefix = '{}'.format(os.path.join(img_save_dir, 'result_valid')) + '_%04d' % i
